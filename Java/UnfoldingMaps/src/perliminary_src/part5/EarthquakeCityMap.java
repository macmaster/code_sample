...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked()
	{
		...
		if(lastClicked instanceof EarthquakeMarker){
			hideMarkers();
			findCityMarkers(cityMarkers);
			lastClicked.setHidden(false);
		}
		else if(lastClicked instanceof CityMarker){
			hideMarkers();
			findEarthquakeMarkers(quakeMarkers);
			lastClicked.setHidden(false);
		}
	}
	
	private void findCityMarkers(List<Marker> markers){
		double radius = ((EarthquakeMarker)lastClicked).threatCircle();
		Location loc = lastClicked.getLocation();
		for(Marker marker : markers){
			if(marker.getDistanceTo(loc)<=radius){
				marker.setHidden(false);
				if(lastClicked instanceof OceanQuakeMarker)
					((OceanQuakeMarker)lastClicked).cities.add(map.getScreenPosition(marker.getLocation()));
			}
		}
	}
	
	private void findEarthquakeMarkers(List<Marker> markers){
		double radius;
		Location loc = lastClicked.getLocation();
		for(Marker marker : markers){
			radius = ((EarthquakeMarker)marker).threatCircle();
			if(marker.getDistanceTo(loc)<=radius){
				marker.setHidden(false);
			}
		}
